...
void init_inputs() {
  ...
  for(i=0; i<SDL_NUM_SCANCODES; i++) {
    in.key_pressed[i]=in.key_count[i]=0;
  }
  for(i=0; i<3; i++) {
...
void check_events() {
  ...
  while(SDL_PollEvent(&event)) {
    ...
    case SDL_KEYDOWN:
      ...
      if(sc==KB_ECHAP) {
        quitter(0);  // Touche Echap, on quitte
      }
    ...
    case SDL_MOUSEBUTTONDOWN:
      ...
      if(0<=b && b<=2) {
        in.mouse_pressed[b]=1;  // On note que le button est pressé
      }
      break;
...
POINT lit_fleches() {
  ...
  // dep.x = dep.y = 0;
  // Touches pressées
  if(in.key_pressed[KB_GAUCHE])	{
    dep.x -= MINDEP;
  }
  if(in.key_pressed[KB_DROITE])	{
    dep.x += MINDEP;
  }
  if(in.key_pressed[KB_HAUT])	{
    dep.y += MINDEP;
  }
  if(in.key_pressed[KB_BAS]) {
    dep.y -= MINDEP;
  }
  // Touches relachées
  dep.x -= MINDEP*in.key_count[KB_GAUCHE];
  in.key_count[KB_GAUCHE] = 0;
  dep.x += MINDEP*in.key_count[KB_DROITE];
  in.key_count[KB_DROITE] = 0;
  dep.y += MINDEP*in.key_count[KB_HAUT];
  in.key_count[KB_HAUT] = 0;
  dep.y -= MINDEP*in.key_count[KB_BAS];
  in.key_count[KB_BAS] = 0;
  return dep;
}
...
BOOL touche_appuyee(int b) {
  if(0<=b && b<SDL_NUM_SCANCODES) {
    return in.key_pressed[b];
  }
  return 0;
}

int touche_nb_appuis(int b)  {
  if(b<0 || b>=SDL_NUM_SCANCODES) {
    return 0;
  }
...
inline void cache_souris() {
  SDL_ShowCursor(0);
}
inline void montre_souris() {
  SDL_ShowCursor(1);
}
inline void capture_souris() {
  SDL_SetRelativeMouseMode(1);
}
inline void libere_souris() {
...
inline void dessine_pixel(POINT p, COULEUR couleur) {
  _pixel(p.x, p.y, couleur);
}
...
inline void dessine_ligne(POINT p1, POINT p2, COULEUR couleur) {
  _ligne(p1.x, p1.y, p2.x, p2.y, couleur);
}
...
inline void dessine_rectangle(POINT p1, POINT p2, COULEUR couleur) {
  _rectangle(p1.x, p1.y, p2.x, p2.y, couleur);
}
...
inline void dessine_rectangle_plein(POINT p1, POINT p2, COULEUR couleur) {
  _rectangle_plein(p1.x, p1.y, p2.x, p2.y, couleur);
}
...
inline void dessine_cercle(POINT centre, int rayon, COULEUR couleur) {
  _cercle(centre.x,centre.y,rayon,couleur);
}
...
void _cercle_plein(int cx, int cy, int rayon, COULEUR couleur) {
  ...
  while(x<=y) {
    ...
    if(d>2*(x-1)) {			   // CAS A
      ...
      x2 = cx+y;
      yh = HEIGHT-(cy+x)-1;
      SDL_RenderDrawLine(SDL_renderer, x1,yh, x2,yh);
      if(x!=0) {
        yh = HEIGHT-(cy-x)-1;
        SDL_RenderDrawLine(SDL_renderer, x1,yh, x2,yh);
      }
...
inline void dessine_cercle_plein(POINT centre, int rayon, COULEUR couleur) {
  _cercle_plein(centre.x, centre.y, rayon, couleur);
}
  ...
  if(verdana_ok) {
    if(a_ecrire) {
      if(fin+strlen(a_ecrire) > 256) {
        fprintf(stderr,"Chaine de caractère trop longue\n");
        return;
      }
...
inline int largeur_image(IMAGE im)  {
  return im->surface->w;
}
inline int hauteur_image(IMAGE im)  {
  return im->surface->h;
}
